function [xPoints, yPoints] = plotSpikeRasterFs(spikes,varargin)
% PLOTSPIKERASTERFS Create raster plot from binary spike data or spike times
...
%
%   Output:
%       xPoints - vector of x points used for the plot. (unit: ms)
...
%
%       AxHandle - default gca (get current axes).
% 
...
%
%       Fs - default 1000 (Hz).
%           Sets the sampling frequency for each timebin for binary spike train data.
% 
%       SpikeDuration - default 1 (millisecond).
%           Sets the horizontal spike length for cell spike time data.
%
%       RelSpikeStartTime - default 0 (ms).
...
%           spike start time of -0.5 ms would center 1 ms spikes for a
...
%           'XLimForCell',[0 10e3],'HorzSpikeLength',0.002,); 
...
fs = p.Results.Fs;
timePerBin = 1000/p.Results.Fs; % ms
...
if islogical(spikes)
    ...
    T = size(spikes,1) * 1000/ fs;
    xlim([0+relSpikeStartTime T+relSpikeStartTime]);
    ylim([0 nTrials+1]);        
    ...
    switch plotType
        ...
        case 'imagesc'
            %% Imagesc
            imagesc(relSpikeStartTime:1/fs:T,[],spikes);
