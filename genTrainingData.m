...
input_spks = spks(:,:,1:end-6); %account for delay
...
Cspks = Cspks(7:end,:); %account for delay
Cspks = [sum(Cspks(:,1:10),2), sum(Cspks(:,11:20),2)];
structured_Cspks = Cspks(:);
smoothed_Cspks = conv(structured_Cspks,kernel,'same');

figure;
plotSpikeRasterFs(logical(Cspks'),'PlotType','vertline2','Fs',1);
xlim([0 temp(1).time(end)])
