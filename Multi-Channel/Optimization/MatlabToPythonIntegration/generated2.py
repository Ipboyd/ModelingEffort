...
def main(trial_number,ps,scale_factor):
    ...
    C_ROn_PSC3_netcon = 1
    dGSYNR1On_On = np.zeros((20))
    dGSYNS1OnOff_On = np.zeros((20))
    dGSYNR1On_S1OnOff = np.zeros((20))
    dGSYNR1Off_S1OnOff = np.zeros((20))
    dGSYNR1Off_Off = np.zeros((20))
    dGSYNS1OnOff_Off = np.zeros((20))
    dGSYNR2On_R1On = np.zeros((20))
    dGSYNS2OnOff_R1On = np.zeros((20))
    dGSYNR2On_S2OnOff = np.zeros((20))
    dGSYNS2OnOff_R1Off = np.zeros((20))
    ...
    On_V = np.ones((20,2)) * [On_E_L, On_E_L]
    On_g_ad = np.ones((20,2)) * [0,0]
    Off_V = np.ones((20,2)) * [Off_E_L, Off_E_L]
    Off_g_ad = np.ones((20,2)) * [0,0]
    R1On_V = np.ones((20,2)) * [R1On_E_L, R1On_E_L]
    R1On_g_ad = np.ones((20,2)) * [0,0]
    R1Off_V = np.ones((20,2)) * [R1Off_E_L, R1Off_E_L]
    R1Off_g_ad = np.ones((20,2)) * [0,0]
    S1OnOff_V = np.ones((20,2)) * [S1OnOff_E_L, S1OnOff_E_L]
    S1OnOff_g_ad = np.ones((20,2)) * [0,0]
    R2On_V = np.ones((20,2)) * [R2On_E_L, R2On_E_L]
    R2On_g_ad = np.ones((20,2)) * [0,0]
    R2Off_V = np.ones((20,2)) * [R2Off_E_L, R2Off_E_L]
    R2Off_g_ad = np.ones((20,2)) * [0,0]
    S2OnOff_V = np.ones((20,2)) * [S2OnOff_E_L, S2OnOff_E_L]
    S2OnOff_g_ad = np.ones((20,2)) * [0,0]
    R1On_On_PSC_s = np.ones((20,2)) * [0,0]
    R1On_On_PSC_x = np.ones((20,2)) * [0,0]
    R1On_On_PSC_F = np.ones((20,2)) * [1,1]
    R1On_On_PSC_P = np.ones((20,2)) * [1,1]
    R1On_On_PSC_q = np.ones((20,2)) * [1,1]
    S1OnOff_On_PSC_s = np.ones((20,2)) * [0,0]
    S1OnOff_On_PSC_x = np.ones((20,2)) * [0,0]
    S1OnOff_On_PSC_F = np.ones((20,2)) * [1,1]
    S1OnOff_On_PSC_P = np.ones((20,2)) * [1,1]
    S1OnOff_On_PSC_q = np.ones((20,2)) * [1,1]
    R1On_S1OnOff_PSC_s = np.ones((20,2)) * [0,0]
    R1On_S1OnOff_PSC_x = np.ones((20,2)) * [0,0]
    R1On_S1OnOff_PSC_F = np.ones((20,2)) * [1,1]
    R1On_S1OnOff_PSC_P = np.ones((20,2)) * [1,1]
    R1On_S1OnOff_PSC_q = np.ones((20,2)) * [1,1]
    R1Off_S1OnOff_PSC_s = np.ones((20,2)) * [0,0]
    R1Off_S1OnOff_PSC_x = np.ones((20,2)) * [0,0]
    R1Off_S1OnOff_PSC_F = np.ones((20,2)) * [1,1]
    R1Off_S1OnOff_PSC_P = np.ones((20,2)) * [1,1]
    R1Off_S1OnOff_PSC_q = np.ones((20,2)) * [1,1]
    R1Off_Off_PSC_s = np.ones((20,2)) * [0,0]
    R1Off_Off_PSC_x = np.ones((20,2)) * [0,0]
    R1Off_Off_PSC_F = np.ones((20,2)) * [1,1]
    R1Off_Off_PSC_P = np.ones((20,2)) * [1,1]
    R1Off_Off_PSC_q = np.ones((20,2)) * [1,1]
    S1OnOff_Off_PSC_s = np.ones((20,2)) * [0,0]
    S1OnOff_Off_PSC_x = np.ones((20,2)) * [0,0]
    S1OnOff_Off_PSC_F = np.ones((20,2)) * [1,1]
    S1OnOff_Off_PSC_P = np.ones((20,2)) * [1,1]
    S1OnOff_Off_PSC_q = np.ones((20,2)) * [1,1]
    R2On_R1On_PSC_s = np.ones((20,2)) * [0,0]
    R2On_R1On_PSC_x = np.ones((20,2)) * [0,0]
    R2On_R1On_PSC_F = np.ones((20,2)) * [1,1]
    R2On_R1On_PSC_P = np.ones((20,2)) * [1,1]
    R2On_R1On_PSC_q = np.ones((20,2)) * [1,1]
    S2OnOff_R1On_PSC_s = np.ones((20,2)) * [0,0]
    S2OnOff_R1On_PSC_x = np.ones((20,2)) * [0,0]
    S2OnOff_R1On_PSC_F = np.ones((20,2)) * [1,1]
    S2OnOff_R1On_PSC_P = np.ones((20,2)) * [1,1]
    S2OnOff_R1On_PSC_q = np.ones((20,2)) * [1,1]
    R2On_S2OnOff_PSC_s = np.ones((20,2)) * [0,0]
    R2On_S2OnOff_PSC_x = np.ones((20,2)) * [0,0]
    R2On_S2OnOff_PSC_F = np.ones((20,2)) * [1,1]
    R2On_S2OnOff_PSC_P = np.ones((20,2)) * [1,1]
    R2On_S2OnOff_PSC_q = np.ones((20,2)) * [1,1]
    R2Off_S2OnOff_PSC_s = np.ones((20,2)) * [0,0]
    R2Off_S2OnOff_PSC_x = np.ones((20,2)) * [0,0]
    R2Off_S2OnOff_PSC_F = np.ones((20,2)) * [1,1]
    R2Off_S2OnOff_PSC_P = np.ones((20,2)) * [1,1]
    R2Off_S2OnOff_PSC_q = np.ones((20,2)) * [1,1]
    R2Off_R1Off_PSC_s = np.ones((20,2)) * [0,0]
    R2Off_R1Off_PSC_x = np.ones((20,2)) * [0,0]
    R2Off_R1Off_PSC_F = np.ones((20,2)) * [1,1]
    R2Off_R1Off_PSC_P = np.ones((20,2)) * [1,1]
    R2Off_R1Off_PSC_q = np.ones((20,2)) * [1,1]
    S2OnOff_R1Off_PSC_s = np.ones((20,2)) * [0,0]
    S2OnOff_R1Off_PSC_x = np.ones((20,2)) * [0,0]
    S2OnOff_R1Off_PSC_F = np.ones((20,2)) * [1,1]
    S2OnOff_R1Off_PSC_P = np.ones((20,2)) * [1,1]
    S2OnOff_R1Off_PSC_q = np.ones((20,2)) * [1,1]
    R2On_R2On_iNoise_V3_sn = np.ones((20,2)) * [0, 0]
    R2On_R2On_iNoise_V3_xn = np.ones((20,2)) * [0, 0]

    #Monitor Declaration
    On_tspike = -1e32*np.ones((20, 5, On_Npop))
    On_buffer_index = np.ones((20))
    On_V_spikes_holder = []
    Off_tspike = -1e32*np.ones((20, 5, Off_Npop))
    Off_buffer_index = np.ones((20))
    Off_V_spikes_holder = []
    R1On_tspike = -1e32*np.ones((20, 5, R1On_Npop))
    R1On_buffer_index = np.ones((20))
    R1On_V_spikes_holder = []
    R1Off_tspike = -1e32*np.ones((20, 5, R1Off_Npop))
    R1Off_buffer_index = np.ones((20))
    R1Off_V_spikes_holder = []
    S1OnOff_tspike = -1e32*np.ones((20, 5, S1OnOff_Npop))
    S1OnOff_buffer_index = np.ones((20))
    S1OnOff_V_spikes_holder = []
    R2On_tspike = -1e32*np.ones((20, 5, R2On_Npop))
    R2On_buffer_index = np.ones((20))
    R2On_V_spikes_holder = []
    R2Off_tspike = -1e32*np.ones((20, 5, R2Off_Npop))
    R2Off_buffer_index = np.ones((20))
    R2Off_V_spikes_holder = []
    S2OnOff_tspike = -1e32*np.ones((20, 5, S2OnOff_Npop))
    S2OnOff_buffer_index = np.ones((20))
