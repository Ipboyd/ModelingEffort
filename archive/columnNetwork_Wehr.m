function [simdata,s] = columnNetwork_Wehr(study_dir,varies,options,netcons)
...
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_inc',0.0001,'tau_ad',180,'t_ref',1.5};
...
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_inc',0.0001,'tau_ad',180,'g_L',1/275,'t_ref',1.5};
...
% s.populations(end).equations = 'noconLIF';
% s.populations(end).size = nCells;
% s.populations(end).parameters = {'t_ref',1.5,'V_reset',-65,'Itonic',0.2,'Imask',Imask};

% s.populations(end+1).name='I';
% s.populations(end).equations = 'noconLIF';
% s.populations(end).size = nCells;
% s.populations(end).parameters = {'t_ref',1.5,'V_reset',-65};
...
EX_rise = 0.5; EX_fall = 3;
XE_rise = 8;  XE_fall = 40;
...
newNetcon = eye(nCells,nCells); newNetcon(2,1) = 1;
...
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.0025,'tauR',EX_rise,'tauD',EX_fall,'fF',0.1,'tauF',80};

s.connections(end+1).direction='X1->R1';
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.003,'tauR',XE_rise,'tauD',XE_fall,'fF',0.1,'tauF',80,'ESYN',-80,'netcon',XRnetcon}; 

s.connections(end+1).direction='X1->S1';
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.003,'tauR',XE_rise,'tauD',XE_fall,'fF',0.1,'tauF',80,'ESYN',-80,'netcon',XRnetcon}; 

SRnetcon = eye(nCells,nCells); SRnetcon(1,2) = 0.7; % SRnetcon(3,3) = 0;

% feed-forward inhibition in L4
s.connections(end+1).direction='S1->R1';
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.02,'tauR',IE_rise,'tauD',IE_fall,'ESYN',-80,'fP',0.5,'tauP',120,'netcon',SRnetcon}; 
...
% s.connections(end).mechanism_list={'PSC'};
% s.connections(end).parameters={'gSYN',0.008,'tauR',1,'tauD',10};
% 
% s.connections(end+1).direction='I->R2';
% s.connections(end).mechanism_list={'PSC'};
% s.connections(end).parameters={'gSYN',0.005,'tauR',1,'tauD',10}; 

Cnetcon = ones(nCells,1); %Cnetcon([1 3]) = 0.5;% Cnetcon(4) = 0.5;

% convergence onto readout cell
s.connections(end+1).direction='R2->C';
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.011,'tauR',EE_rise,'tauD',EE_fall,'netcon',Cnetcon};
