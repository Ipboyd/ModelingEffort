function [simdata,s] = columnNetwork_oneStage(study_dir,varies,options,netcons)
...
s.populations(1).parameters = {'t_ref',0.5};
...
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_L',1/100,'E_L',-57,'V_reset',-50,'t_ref',0.8};

s.populations(end+1).name='R1';
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_inc',0.0001,'tau_ad',120,'t_ref',1.5};

s.populations(end+1).name='X1';
s.populations(end).equations = 'noconLIF';
s.populations(end).size = nCells;
s.populations(end).parameters = {'g_L',1/275,'t_ref',1.5};
...
s.connections(end).mechanism_list={'PSC'};
s.connections(end).parameters={'gSYN',0.0015,'tauR',EX_rise,'tauD',EX_fall,'fF',0.1,'tauF',80};
