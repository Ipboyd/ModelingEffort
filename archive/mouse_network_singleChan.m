function [perf, annotstr] = mouse_network_singleChan(study_dir,time_end,varies,plot_rasters,plot_title)
...
for vv = 1:jump %2nd varied variable
    ...
    if plot_rasters
        ...
        for i = 1:4 %for each spatially directed neuron
            ...
            perf(i,vv) = calcPCandPlot(thisRaster,time_end,1);        
            if i==1, ylabel('R'); end
            xticklabels([])
        end
        ...
        for aa = 1:length(varies)-1
            annotstr{vv,aa} = sprintf('%s = %.3f',paramstr{aa},eval(['data(' num2str(vv) ').' paramstr{aa}]));
        end
