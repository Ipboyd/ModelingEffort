function [simdata,s] = columnNetwork_V3(study_dir,varies,options,netcons)
...
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.011,'tauR',EE_rise,'tauD',EE_fall,'t_ref',1.5};

s.connections(end+1).direction='IC->S1';
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.02,'tauR',EI_rise,'tauD',EI_fall,'fP',0.2,'tauP',80,'t_ref',0.8};

% SOM inhibition
s.connections(end+1).direction='R1->X1';
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.0025,'tauR',EX_rise,'tauD',EX_fall,'fF',0.1,'tauF',180,'t_ref',1.5};

s.connections(end+1).direction='X1->R1';
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.003,'tauR',XE_rise,'tauD',XE_fall,'fF',0.1,'tauF',180,'ESYN',-80,'netcon',XRnetcon,'t_ref',1.5}; 

% feed-forward inhibition in L4
s.connections(end+1).direction='S1->R1';
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.02,'tauR',IE_rise,'tauD',IE_fall,'ESYN',-80,'fP',0.5,'tauP',120,'t_ref',1.5}; 
...
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.02,'tauR',EI_rise,'tauD',EI_fall,'fP',0.2,'tauP',80,'t_ref',0.8}; %gsyn = 0.025 for 1:1 spiking
...
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.011,'tauR',EE_rise,'tauD',EE_fall,'netcon',ones(nCells,1),'t_ref',1.5};

s.connections(end+1).direction='S2->C';
s.connections(end).mechanism_list={'PSC2'};
s.connections(end).parameters={'gSYN',0.02,'tauR',IE_rise,'tauD',IE_fall,'ESYN',-80,'fP',0.5,'tauP',120,'netcon',ones(nCells,1),'t_ref',1.5}; 
