...
% 3. mean_rate, double check this after simulation since noise in poisson
% generator can affect the actual rate generated.
...
noisePSTHsongloc = 3;
noisePSTHmaskloc = 3;
azimuth=[90 45 0 -90]; %experiment stimuli locations
...
% tuningcurve(3,:)= sigmf(x,[0.016 82.5])-0.05; %sigmodial
tuningcurve(4,:)= sigmf(x,[0.016 -22.5])-0.05; %sigmodial
neuronNames = {'ipsi sigmoid','gaussian','U','contra sigmoid'};
% neuronNames = {'left sigmoid','gaussian','inh right sigmoid','right sigmoid'};

figure
plot(x,tuningcurve')
legend(neuronNames)
% temp = tuningcurve(4,:) - tuningcurve(3,:);
% hold on;
% plot(x,temp)
% legend([neuronNames,'difference of sigmoids'])
...
for songn = 1:2
    ...
    for songloc = 1:4
        maskerloc = 0;
        for channel = 1:4 % "spatial" channel
            if songloc, songWeight = tuningcurve(channel,x==azimuth(songloc)); end
            PSTH_clean{songn,songloc}(:,channel) = songPSTH*songWeight;
        end
    end
end
    ...
    for songloc = 1:4
        ...
        for songn = 1:2
            ...
            for channel = 1:4 % "spatial" channel
                ...
                if maskerloc, maskerWeight = tuningcurve(channel,x==azimuth(maskerloc)); end
                if songloc, songWeight = tuningcurve(channel,x==azimuth(songloc)); end
                ...
                PSTH_mixed{songn,songloc,maskerloc}(:,channel) = mixed_rate;
%                 end
            end
        end
    end
...
for channel = 1:4
    ...
    for songloc = 1:4
        subplot(1,4,songloc);
        plot(PSTH_clean{songn,songloc}(:,channel));
        ylim([0 10])
    end
    sgtitle(neuronNames(channel));
end
...
for channel = 1:4
    ...
    for songloc = 1:4
        for maskerloc = 1:4
            subplot(4,4,songloc+(maskerloc-1)*4);
            plot(PSTH_mixed{songn,songloc,maskerloc}(:,channel));
            ylim([0 10])
        end
    end
    sgtitle(neuronNames(channel));
...
addpath('genlib')
numTrials = 10;
locLabels = strtrim(cellstr(num2str(azimuth'))');
...
for songloc = 1:4
    ICspks_clean(songloc).spks = zeros(20,4,3000/dt);

    for channel = 1:4
        tempSpkTimes = [];
        for songn = 1:2
            [spike_times, spkcnt] = PSTH2Spks(PSTH_clean{songn,songloc}(:,channel),numTrials,PSTHgain,dt/1000,noiseMultiplier);
        ...
        for ii = 1:10
            for jj = 1:2
                currentSpkTimes = round(tempSpkTimes{ii,jj})/dt;
                ICspks_clean(songloc).spks(ii + 10*(jj-1),channel,currentSpkTimes) = 1;
            end
        end    
    end
    ICspks_clean(songloc).fr = mean(sum(ICspks_clean(songloc).spks,3)/3);
end

for songloc = 1:4
    for maskloc = 1:4
        ICspks_mixed(songloc,maskloc).spks = zeros(20,4,3000/dt);

        for channel = 1:4
            tempSpkTimes = [];
            for songn = 1:2
                [spike_times, spkcnt] = PSTH2Spks(PSTH_mixed{songn,songloc,maskloc}(:,channel),numTrials,PSTHgain,dt/1000,noiseMultiplier);
                tempSpkTimes = [tempSpkTimes, spike_times];
            end
             distMat = calcvr(tempSpkTimes, tau);
            [performance, ~] = calcpc(distMat, numTrials, 2, 1,[], 'new');
        ...
        ICspks_mixed(songloc,maskloc).fr = mean(sum(ICspks_mixed(songloc,maskloc).spks,3)/3);
    end
end
...
params.noiseMultiplier = noiseMultiplier;
params.sigma = sigma;
...
PSTH.song = PSTH_song;
PSTH.noise = PSTH_noise;
PSTH.mixed = PSTH_mixed;

filename = sprintf('data_recordings\\reconfigured\\%s %sTMR %sdB chan%i noise%i gain%i.mat',subjectID,TMR,laserPow,expChannel,noiseMultiplier,PSTHgain);
save(filename,'ICspks_mixed','ICspks_clean','meta','params','PSTH')
