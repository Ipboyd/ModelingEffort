...
s.connections(end).parameters={'gSYN',0.175, 'tauR',0.4, 'tauD',20, 'netcon', ones(nPops,1),'ESYN',-80,'delay',2}; 
...
clear FR_X FR_C FR_R

for i = 1:length(gSYN_range)
    FR_X(i) = mean(sum(data(i).X_V_spikes))/0.5;
    FR_Y(i) = mean(sum(data(i).X_V_spikes))/0.5;
    FR_R(i) = sum(data(i).R_V_spikes)/0.5;
%     FR_C(i) = sum(data(i).C_V_spikes)/0.5;
end
figure; scatter(gSYN_range,FR_R)
xlabel('gSYN (itonic = 0.35, FR = 34)')
