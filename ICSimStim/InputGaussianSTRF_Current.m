function [t_spiketimes_on,t_spiketimes_off]=InputGaussianSTRF_Current...
...
%             masker = wgn(1,n_length,1);
            ...
            [frate_on,frate_off] = STRFconvolve_Current(strf,currspec,mean_rate,offsetFrac);
        ...
        if songn==1
            ...
            plot(t,frate_on,'linewidth',1); xlim([0 max(t)]); hold on;
            plot(t,frate_off,'linewidth',1);
        end
                                
        %Below section gives the whole row of top labels
        if songn==2
            %firingRate = round(sum(cellfun(@length,t_spiketimes_on(:,i+4)))/10/(max(t)-0.250));
            %title({neuronNames{i},['FR = ',num2str(firingRate)]})
            title(neuronNames{i});
        end
    ...
    if saveParam.flag
        ...
        save([savedir '/s' num2str(songloc) 'm' num2str(maskerloc)],'songloc','maskerloc',...
            'sigma','mean_rate','fr_on','fr_off')
    end
end
clf