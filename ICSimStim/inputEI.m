function [t_spiketimes_on]=inputEI(specs,songloc,maskerloc,tuning,saveParam,...
    mean_rate,stimGain,maxWeight,paramSpk)
% Inputs
...
% tuningcurve(1,:) = sigmf(x,[0.022 45])/sigmf(90,[0.022 45]); % = 1 at x = +90
tuningcurve(1,:) = 0.8*sigmf(x,[0.07 15])/sigmf(90,[0.07 15]) + 0.2; % = 1 at x = +90
...
tuningcurve(4,:) = 0.8*sigmf(x,[-0.07 -15])/sigmf(-90,[-0.07 -15]) + 0.2; % at -90°, sigmoid == 1

% 5th tuning curve - shifted sigmoid (inhibitory only)
